...
function importReport(value){if(value?.format!=='SL_WORLD_ENGINE_LAB'||value.schemaVersion!==1||!Array.isArray(value.groups)||!value.groups.length||value.groups.length>3)throw Error('対応するLAB JSON（schemaVersion 1）ではありません');for(const g of value.groups){L.validate(g.bundle.scenario);if(!Array.isArray(g.trials)||!g.trials.length||!Array.isArray(g.bundle.teams))throw Error('再現に必要なtrial / 選手snapshotがありません');for(const t of g.trials)if(typeof t.seed!=='string'||!t.decision||!t.execution||!t.outcome||!Array.isArray(t.outcome.finalScore)||t.outcome.finalScore.length!==2||t.outcome.finalScore.some(n=>!Number.isFinite(n))||!Array.isArray(t.anomalies))throw Error('trial詳細が不完全です');g.summary=L.summarize(g.trials);}value.anomalies??=[];value.savedSeeds??=[];return value;}
...
globalThis.SL_LAB_PRESENTATION={labels,label,title,situation,axisNames,defaultAxes,condition,metric,metrics,candidates,diagnosis,nextTest,comparison,beforeAfter,analysis,exportReport,importReport,filename};
})();
