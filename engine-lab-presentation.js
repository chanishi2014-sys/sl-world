...
const condition=(g,i)=>({id:String.fromCharCode(65+i),label:g.changed?.axisB?String(g.changed.value)+' × '+String(g.changed.valueB):g.changed?.value!=null?String(g.changed.value)==='none'?'なし':String(g.changed.value):g.label||'単一条件'});
...
function nextTest(r){const s=copy(r.groups[0].bundle.scenario);if(r.axisB)return {label:'同じ2軸マトリクスを再検証',reason:'固定条件を保持して同じ2軸を準備します。',scenario:s,axis:r.axis,axisB:r.axisB,trialCountPerCondition:r.trialCount,autoRun:false};let axis,name;
...
function comparison(r){
 const base=r.groups[0].bundle,spec=L.comparisonSpec(r.axis,r.axisB),snapshot=b=>({scenario:b.scenario,teams:b.teams});
 const perCondition=r.groups.map(g=>differentPaths(snapshot(base),snapshot(g.bundle))),paths=[...new Set(perCondition.flat())],inputs=r.groups.map(g=>L.effectiveInputs(g.bundle));
 const fixed=Object.fromEntries(Object.entries(inputs[0]).filter(([key,value])=>!spec.axes.some(a=>a.id===key)&&inputs.every(x=>JSON.stringify(x[key])===JSON.stringify(value))));
 return {axis:r.axis,...(r.axisB?{axisB:r.axisB}:{}),mode:spec.mode,axes:spec.axes,conditionCount:r.groups.length,target:L.axes[r.axis]?.role||null,ability:L.axes[r.axis]?.stat||(L.axes[r.axis]?.trait?'specials':r.axis==='fatigue'?'pitchCount':null),label:spec.axes.length?spec.axes.map(a=>axisNames[a.id]||a.id).join(' × '):axisNames.none,
 conditions:r.groups.map((g,i)=>({...condition(g,i),value:g.changed?.value??null,...(g.changed?.axisB?{valueB:g.changed.valueB}:{}),scenario:g.bundle.scenario,effectiveInputs:inputs[i],changedPaths:perCondition[i]})),
 fixedConditions:{referenceCondition:'A',values:fixed,changedPaths:paths,description:paths.length?'A条件の保存snapshotに対し、changedPaths以外は同一':'比較条件間の変更なし',referenceBundle:base}};
}
...
function solReport(r,before=null){
 ...
 if(t.representativeProcess){detail.representativeProcess=[];omittedRecords.representativeProcess=0;for(const process of t.representativeProcess){detail.representativeProcess.push(process);if(bytes(detail)>limits.detailBytesPerSeed){detail.representativeProcess.pop();omittedRecords.representativeProcess++;}}}
...
function importReport(value){if(value?.format==='SL_WORLD_ENGINE_LAB_SOL')throw Error('ソル解析用JSONは共有用の軽量形式です。履歴・seed再生の移行には「完全ログJSON」を読み込んでください。');if(value?.format!=='SL_WORLD_ENGINE_LAB'||value.schemaVersion!==1||!Array.isArray(value.groups)||!value.groups.length||value.groups.length>9)throw Error('対応するLAB JSON（schemaVersion 1）ではありません');if(value.axisB?L.comparisonSpec(value.axis,value.axisB).conditionCount!==value.groups.length:value.groups.length>3)throw Error('比較軸と条件数が一致しません');for(const g of value.groups){L.validate(g.bundle.scenario);if(!Array.isArray(g.trials)||!g.trials.length||!Array.isArray(g.bundle.teams))throw Error('再現に必要なtrial / 選手snapshotがありません');for(const t of g.trials)if(typeof t.seed!=='string'||!t.decision||!t.execution||!t.outcome||!Array.isArray(t.outcome.finalScore)||t.outcome.finalScore.length!==2||t.outcome.finalScore.some(n=>!Number.isFinite(n))||!Array.isArray(t.anomalies))throw Error('trial詳細が不完全です');const aggregateReason=value.storage?.mode==='aggregate-and-thin-trials'?g.summary?.reason:null;g.summary=L.summarize(g.trials);if(aggregateReason)g.summary.reason=aggregateReason;}value.anomalies??=[];value.savedSeeds??=[];return value;}
function filename(r){const safe=x=>String(x||'test').replace(/[^\p{L}\p{N}_.-]/gu,'-').slice(0,70);return `sl-world-lab_${safe(r.testId||r.seedPrefix)}_${safe((r.groups[0].bundle.scenario.id||'custom')+'-'+[r.axis,r.axisB].filter(Boolean).join('-x-').replace(/[A-Z]/g,m=>'-'+m.toLowerCase()))}_${safe(r.date?.slice(0,10))}.json`;}
