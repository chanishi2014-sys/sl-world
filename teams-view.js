...
function render(root){
 ...
 function draw(){
  ...
  if(state.playerId==null){root.dataset.screen='team';root.append(button(league.id+'所属チーム一覧へ戻る','teams-back',list));title(club.currentName);
   ...
   for(const id of ids){const p=roster.byId.get(id),b=button(p.name||'氏名未登録','player',()=>move({playerId:id}));b.className='clubOpen player-open';b.dataset.playerId=String(id);b.dataset.focusKey='player:'+typeof id+':'+id;section.append(b);}
  ...
  for(const type of BASIC){const value=p.batting?.[type],spec=D.describeAbility(type,value),cell=node('div','player-ability-card');cell.dataset.ability=type;
...
globalThis.SL_TEAMS_VIEW=Object.freeze({readRoster,render});
})();
