...
(() => {
 ...
 function resolveBunt(g,pitcher,batter){
  ...
  p.buntContactPrecision=contact;p.buntPopRisk=.08+.12*(1-meet)+.08*(.5-p.buntQuality);p.buntPop=g.rng()<p.buntPopRisk;p.outcome='inPlay';p.battedResult=null;p.battedQuality=p.buntQuality;p.battedGrade='touch';p.physical={exitVelocity:p.buntPop?8:8+5*(1-p.buntQuality),launchAngle:p.buntPop?65:0,sprayAngle:(g.rng()<.5?-1:1)*(.22+(g.rng()*2-1)*(.5-.3*p.buntQuality)),type:p.buntPop?'FLY':'GROUND',hangTime:p.buntPop?1.5:0,horizontalSpeed:p.buntPop?18:65+30*(1-p.buntQuality),deceleration:65,height:p.buntPop?15:0};return p;
 }
