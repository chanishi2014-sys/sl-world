...
(() => {
 ...
 function offenseDecision(g,b,p){
  ...
  const candidates=[swing,bunt,...steals,hitRunOption(g,b)];
  let selected;
  ...
  return {...selected,intent:selected.decision==='SWING_AWAY'?'score more runs':'runner advance',context:context(g),candidates:candidates.map(c=>candidateTrace(c,swing)),policy:'shared-alpha1'};
 }
 ...
 function execute(g,p,b,d){if(d?.decision==='SAC_BUNT')return resolveBunt(g,p,b);if(d?.decision!=='HIT_AND_RUN')return resolvePitch(g,p,b);const effects=specialModifiers(b,p),pitch=generatePitchQuality(g,p,effects);Object.assign(pitch,judgePitch(g,pitch,effects),{swing:true});Object.assign(pitch,resolveContact(g,b,pitch,effects));if(pitch.outcome==='inPlay')Object.assign(pitch,resolveFinalResult(g,b,pitch,generateBattedQuality(g,b,pitch,effects),effects));pitch.hitAndRun=true;if(pitch.outcome==='swingingStrike'&&F().canSteal(g,1))pitch.runningExecution=F().steal(g,p,{fromBase:1});return pitch;}
 globalThis.SL_TACTICS={hitRunOption,execute,enabled,context,buntOption,stealOption,offenseDecision,defenseDecision,groundPlay,resolveBunt,buntGeometry,eventTrace};
})();
