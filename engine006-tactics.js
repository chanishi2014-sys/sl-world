...
(() => {
 ...
 function defenseDecision(g,p,b,{settled=false}={}){
  ...
  const candidates=[1,2,3,4].map(base=>{
   ...
   const fieldTime=p.bunt?1.1+Math.hypot(point[0]-alignment(g).positions[p.fielderIndex][0],point[1]-alignment(g).positions[p.fielderIndex][1])/90+.8*(p.buntQuality??.5):.8+Math.hypot(point[0]-400,point[1]-430)/210+Math.hypot(point[0]-alignment(g).positions[p.fielderIndex][0],point[1]-alignment(g).positions[p.fielderIndex][1])/150+(1-skill)*.25;
